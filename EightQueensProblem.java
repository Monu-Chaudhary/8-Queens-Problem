...
public class EightQueensProblem {
	public final static int N = 8; // Size of the board, feel free to change it to test your implementation against
									// larger boards
	...
	public void minConflicts() {
		while(!isSolved()) {
			...
			int minRow = 0, minVal = N;
			for (int i= 0; i<N; i++) {
				conflict[i] = conflicts(i, col);
				if (minVal > conflict[i] && i != row) {
					minRow = i;
					minVal = conflict[i];
				}
			}
			
			int col2 = queens[minRow];
			queens[minRow] = col;
			queens[row] = col2;
		}
	...
	public boolean contains(int col, int j) {
		for (int i = 0; i < j; i++) {
			if (queens[i] == col)
				return true;
		}
